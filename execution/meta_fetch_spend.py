...
Uses account-level insights with an explicit AEST date range to ensure the
correct day is always reported, regardless of Meta account timezone.
...
def fetch_yesterday_spend() -> dict:
    ...
    Uses explicit AEST date to ensure correct day regardless of account timezone.

    Returns:
        dict with keys:
            date      (str)   — Yesterday's date YYYY-MM-DD (AEST)
    ...
    # Use AEST yesterday date explicitly (not Meta's own "yesterday" preset)
